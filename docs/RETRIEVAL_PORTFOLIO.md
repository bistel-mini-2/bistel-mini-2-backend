...
하이브리드는 각 채널에서 `top_k × 2`개 후보를 가져온 뒤 다음 weighted
RRF 점수를 청크별로 합산한다. Vector 가중치는 1.0, SQL 가중치는 0.25로
설정했다.

```text
weighted RRF score = Σ channel_weight / (60 + rank)
```

전체 정책을 탐색할 때는 동일 정책의 청크를 최대 2개까지만 최종 Top K에
...
Section Hit@5가 **68%p** 높았다. 초기 동등 가중치 하이브리드는 Vector와
Hit@5가 같고 MRR이 낮았지만, weighted RRF와 정책별 청크 제한을 적용한
뒤 Policy Hit@5가 Vector보다 **2%p**, MRR이 **0.0260** 높아졌다.
Section Hit@5는 80%로 유지했다.
...
복수 정답으로 수정했다. 따라서 이 평가는 정책 원문 기준의 50문항 내부
검색 검증셋으로 설명할 수 있다. 다만 외부 평가자 간 일치도를 측정한
공개 표준 데이터셋은 아니다.
...
- 정책과 문항 범위가 제한된 내부 검색 검증셋이므로 전체 복지정책
  도메인의 일반화 성능을 의미하지 않는다.
...
- 다음 단계에서는 같은 50문항에 정답 답변 또는 원자적 사실 목록을
  ...
  평가해야 한다.
- 하이브리드는 한국어 토큰화나 PostgreSQL FTS, 정책 상세 청크 가중치를
  적용한 뒤 동일 골드셋으로 다시 비교할 수 있다.
