...
| pgvector | 86.0% | 78.0% | 0.7497 | 782ms | 1,093ms |
| Weighted Hybrid RRF | **88.0%** | **78.0%** | **0.7757** | 1,292ms | 1,717ms |
| Adaptive fallback | 86.0% | 78.0% | 0.7497 | 809ms | 2,290ms |

확인된 결과는 다음과 같다.

- pgvector는 단순 SQL keyword 기준선보다 Policy Hit@5가 68%p,
  Section Hit@5가 66%p 높았다.
...
- Adaptive는 broad 검색에서 7/50문항(14%)만 SQL fallback을 실행하면서
  Vector와 같은 Policy Hit@5 86%, Section Hit@5 78%를 유지했다.
...
수 있는 문항은 복수 정답으로 수정했다. 다만 내부 AI 기반 검수이며, 외부
평가자 간 일치도를 측정한 사람 검수 데이터는 아니다.
...
- **“검증된 표준 데이터셋을 사용했다”**
  - 프로젝트 정책 데이터로 만든 내부 골드셋이며 독립적인 사람 검수는
    완료되지 않았다.
- **지연시간을 운영 SLA처럼 표현**
