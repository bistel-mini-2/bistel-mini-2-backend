...
- id: policies_list
  ...
  owner: "탐색/상세"
  query_params:
    query: "string?; 기본 정책명 검색. q와 둘 다 있으면 query 우선"
    q: "string?; query의 이슈 #29 호환 별칭. 둘 다 있으면 query 우선"
    search_scope: "policy_name | name | title | all; 기본값 policy_name. all이면 카테고리·상세 내용·태그까지 확장 검색"
    searchScope: "search_scope의 camelCase 별칭"
    detail_q: "string?; 상세 조건 검색어. 정책명 검색과 분리되어 대상/지원내용/신청방법/조건 프로필/태그를 추가로 검색"
  ...
  response_schema:
    data:
      - policy_id: "string; 내부 bigint PK의 문자열 표현"
        ...
        target_stage_display: "string[]; 화면 표시용 생애단계 라벨"
        life_stage_display: "string?; 화면 표시용 생애단계 요약. 전체 대상이면 '모든 생애 단계 대상'"
        all_age: "boolean; 전 연령 대상으로 판단되면 true"
        display_age: "string?; 화면 표시용 연령/생애단계 문구"
        summary: "string?"
        target_summary: "string?; 사용자용 대상 요약"
        benefit_summary: "string?"
        benefit_summary_display: "string?; 사용자용 지원 내용 요약"
        agency: "string?"
        benefit_type: "string?"
        application_status: "string?"
        application_status_display: "string?; 화면 표시용 신청 상태"
        ...
        region_display: "string?; 화면 표시용 지역 문구"
        official_url: "string?"
    meta:
      page: "integer"
      size: "integer"
...
- id: policies_detail
  ...
  response_schema:
    ...
    data:
      ...
      official_url: "string"
      target_summary: "string?"
      benefit_summary_display: "string?"
      application_summary: "string?"
      application_guide:
        available_display: "string?"
        channel: "온라인 | 방문 | 온라인 또는 방문 | 우편/팩스?"
        route: "string?"
      ...
      related:
        - policy_id: "string"
          ...
          tagTone: "string"
          icon: "string?"
          related_reason: "string?; 같은 분야/생애 단계/지역/태그 등 추천 사유"
    error: null
    meta: {}
